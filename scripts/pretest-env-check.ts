...
function parseVersion(versionString: string): number[] {
  return versionString.replace(/^v/, '').split('.').map(n => parseInt(n, 10));
}
...
async function checkServices(): Promise<CheckResult> {
  ...
  return {
    ...
    details: Object.entries(serviceResults).map(([service, available]) => 
      `${available ? '✅' : '❌'} ${service}`
    )
  };
}
...
async function runChecks(options: { fix?: boolean; verbose?: boolean } = {}): Promise<void> {
  ...
  if (failed > 0) {
    ...
    console.log('⚠️ Environment validation completed with warnings. Tests may run but some features might not work optimally.');
